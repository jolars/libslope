...
# coverage is not supported by the compiler or disabled. This function must be
# defined here, because the module will be exited, if there is no coverage
...
if(GENINFO_BIN AND NOT DEFINED GENINFO_EXTERN_FLAG)
  ...
  set(
    ...
    "${FLAG}"
    CACHE STRING
    "Geninfo flag to exclude system sources."
  )
endif()
...
function(lcov_merge_files OUTFILE ...)
  ...
  add_custom_command(
    OUTPUT "${OUTFILE}"
    COMMAND
      ${LCOV_BIN} --quiet -a ${OUTFILE}.raw --output-file ${OUTFILE}
      --base-directory ${PROJECT_SOURCE_DIR} ${LCOV_EXTRA_FLAGS}
    COMMAND
      ${LCOV_BIN} --quiet -r ${OUTFILE} ${LCOV_REMOVE_PATTERNS} --output-file
  ...
  foreach(FILE ${SOURCES})
    ...
    add_custom_command(
      ...
      COMMAND
        ...
        ${PROJECT_SOURCE_DIR} --initial --gcov-tool ${GCOV_BIN}
        --output-filename ${OUTFILE} ${GENINFO_EXTERN_FLAG} ${TDIR}/${FILE}.gcno
      DEPENDS ${TNAME}
...
function(lcov_capture_tgt TNAME)
  ...
  foreach(FILE ${SOURCES})
    ...
    # Create an empty .gcda file, so the target capture file can have a
    # dependency on it. The capture file will only use this .gcda if it has a
    # non-zero size (test -s).
    add_custom_command(
      OUTPUT "${TDIR}/${FILE}.gcda"
    ...
    add_custom_command(
      OUTPUT ${OUTFILE}
      COMMAND
        test -s "${TDIR}/${FILE}.gcda" && ${GCOV_ENV} ${GENINFO_BIN} --quiet
        --base-directory ${PROJECT_SOURCE_DIR} --gcov-tool ${GCOV_BIN}
        --output-filename ${OUTFILE} ${GENINFO_EXTERN_FLAG} ${TDIR}/${FILE}.gcda
  ...
  add_custom_target(
    ${TNAME}-genhtml
    COMMAND
      ${GENHTML_BIN} --quiet --sort ${GENHTML_CPPFILT_FLAG} --prefix
      ${PROJECT_SOURCE_DIR} --baseline-file ${LCOV_DATA_PATH_INIT}/${TNAME}.info
      --output-directory ${LCOV_HTML_PATH}/${TNAME} --title
      "${CMAKE_PROJECT_NAME} - target ${TNAME}" ${OUTFILE}
...
function(lcov_capture)
  ...
  if(NOT TARGET lcov)
    ...
    add_custom_target(
      lcov
      COMMAND
        ${GENHTML_BIN} --quiet --sort ${GENHTML_CPPFILT_FLAG} --baseline-file
        ${LCOV_DATA_PATH_INIT}/all_targets.info --output-directory
        ${LCOV_HTML_PATH}/all_targets --title "${CMAKE_PROJECT_NAME}" --prefix
...
if(NOT TARGET lcov-genhtml)
  add_custom_target(
    lcov-genhtml
    COMMAND
      ${GENHTML_BIN} --quiet --output-directory
      ${LCOV_HTML_PATH}/selected_targets --title \"${CMAKE_PROJECT_NAME} -
      targets `find ${LCOV_DATA_PATH_CAPTURE} -name \"*.info\" ! -name
      \"all_targets.info\" -exec basename {} .info \\\;`\" --prefix
      ${PROJECT_SOURCE_DIR} --sort ${GENHTML_CPPFILT_FLAG} `find
      ${LCOV_DATA_PATH_CAPTURE} -name \"*.info\" ! -name \"all_targets.info\"`
  )
